...
# ✅ Landon’s “main” select list (exact)
SELECT_FIELDS = [
    "TEMPLOYEE_UUID",
    "CEMPLOYEE_UUID",
    "C0DATEFROM",
    "C0DATETO",
    "KCLEAVERS",
    "COCHAR_STRUCTURE",   # include so it shows in the output
]
...
RENAME_MAP = {
    ...
    "COCHAR_STRUCTURE": "COCHAR_STRUCTURE",
}

# ✅ Landon’s structure field name
...
# ---------------------- Core ETL ----------------------
def fetch_distinct_structures(top: int = 1000000) -> List[str]:
    url = _entity_url(SAP_CODES_QUERY)
    params = {"$select": STRUCT_FIELD, "$top": str(top), "$format": "json"}
...
def fetch_rows_for_structure(structure: str, top_per_page: int = 1000000) -> List[Dict]:
    """
    Pull rows for one structure. If SAP complains about a select field,
    auto-drop it and retry.
    """
    base_url = _entity_url(SAP_MAIN_QUERY)
    working_fields = list(SELECT_FIELDS)

    # escape single quotes for OData
    filter_value = structure.replace("'", "''")
    ...
    while True:
        ...
        if resp.ok:
            ...
            # follow paging if server provides it
            while next_link:
            ...
            logging.info("  %s -> %d rows", structure, len(all_rows))
            return all_rows
        ...
        logging.error("HTTP %s for %s params=%s\nBody: %s",
                      resp.status_code, base_url, params, resp.text[:2000])
        resp.raise_for_status()
